...
//            括号被点击
            btnBracket.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
//                    如果计算完毕，设定当前数学表达式为空，并且设置equal_flag为0，方便之后进行计算
                    if (equal_flag == 1) {
                        mathCurrent = "";
                        equal_flag = 0;
                    }
//                    当前表达式长度为0,说明什么都没有输入，那么直接加"("
                    if (mathCurrent.length() == 0) {
                        mathCurrent += "(";
                    }
//                    最后一个字符是字符并且不是")",直接加"("
                    else if (baseCalculator.isOper(mathCurrent.charAt(mathCurrent.length() - 1)) && mathCurrent.charAt(mathCurrent.length() - 1) != ')') {
                        mathCurrent += "(";
                    }
//                      最后一位是数字或者是'π'或者是'e'
...
//                        如果当前的表达式没有"(",当前表达式加上"*(",其他则+= ")"
                        if (!hasStartBracket(mathCurrent))
                            mathCurrent += "*(";
                        else
                            mathCurrent += ")";
                    }
//                    如果当前表达式最后一位是")",当前字符串加上")"
